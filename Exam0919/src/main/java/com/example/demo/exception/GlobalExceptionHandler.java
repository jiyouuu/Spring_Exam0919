...
public class GlobalExceptionHandler {
	...
	public String handleWaffleBearException(WaffleBearExperimentException e, Model model) {
	 ...
	 // 유효성 검사 실패 처리 MethodArgumentNotValidException
	 @ExceptionHandler(MethodArgumentNotValidException.class)
	 public String handleValidationException(MethodArgumentNotValidException e, Model model) {
		 // BindingResult와 DTO를 뷰에 전달
		 model.addAttribute("org.springframework.validation.BindingResult.userResistrationDto", e.getBindingResult());
		 model.addAttribute("userRegistrationDto",  e.getBindingResult().getTarget());
		 return "auth/register";
	 }
